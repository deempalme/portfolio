...
$(document).ready(function() {
	...
	t1 = '01575', e1 = 'f', e2 = 'j', e3 = 'ramirez', e4 = 'rodriguez', t2 = ' 140 15 00', m1 = 'gm', m2 = 'ail', play = [1, 1, 1, 1],
	...
	pos3aulul = pos3a.children('ul').children('li').children('ul').children('li'), pos1a1h1 = pos1a1.find('h1'), poh6 = po.find('h6'),
	pos2h6 = poh6.eq(0), pos3h6 = poh6.eq(1);
	...
	for(i = 1; i < 3; i++){
		obP[i] = document.createElement('div');
		obP[i].setAttribute('class','obP'+i);
		pe.append(obP[i]);
	}
	...
	function mvtP(nu){
		if(mdpT == 0){
			...
			switch(nu){
				...
				$('body, html').animate({ scrollTop:porT3 }, 5000, function(){ mdpT = 0; });
				break;
				case 2:
				offMouse();
				stopV();
				...
				$('body, html').animate({ scrollTop:porT3 }, 2000, function(){ mdpT = 0; });
				break;
				case 7:
				offMouse();
				playV();
	...
	$(window).scroll(function(e){
		...
		if(scT < (h1*3.2)){
			...
			$(div2[0]).css('top',pY1+'px');
			$(div[pts + 3]).css('top',pY2+'px');
			$(div[pts + 4]).css('top',pY3+'px');
			hea.css('top',pYl+'px');
			pXY(hesa, pY4, 0, 1, 3);
		...
		if(scT > porT2){
			if(porM == 0){
				pos1a3.css('left',(w1*0.2-74)+'px');
				porM = 1;
			}
		}else{
			if(porM == 1){
				pos1a3.css('left',(w12-74)+'px');
				porM = 0;
			}
		}
		if(scT >= conT3){
