...
$(document).ready(function() {
  ...
  for(i = 1; i <= (pts + 4); i++){
    ...
    if(i <= pts){
      ...
      obj[i].setAttribute('src', 'theme/p'+i+'.png')
  ...
  pls[1].setAttribute('src','theme/pl2.gif');
  ...
  pls[2].setAttribute('src','theme/pl2.gif');
  ...
  pos2afbp.click(function(){
    ...
    if(play[i] == 0){
      $(this).children('img').attr('src','theme/pause.png');
      $(this).closest('figure').children('video').get(0).play();
      play[i] = 1;
    }else{
      $(this).children('img').attr('src','theme/play.png');
